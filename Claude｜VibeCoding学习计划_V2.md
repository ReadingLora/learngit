...
- [ ] 测试：刷新页面后还能看到之前输入的块
- [ ] **右栏笔记可编辑并写回存储**（与 `reader-layout` 原型衔接）：展开后正文可改；**失焦或按钮「保存」**（二选一即可）将修改写回 `localStorage` 中对应条目；刷新后仍为修改后的内容。折叠标题取「第一行非空」规则在编辑后需同步更新（或约定保存时重算标题）。
...
- [x] ~~写 `reader-web-prototype-review.md`~~（不做，已由 Week 3 复盘覆盖）
- [x] 列出网页原型目前能做什么、不能做什么
- [x] **写 Week 3 复盘**

**Week 3 验收**：
- [x] 拥有一个可保存、可导出 Markdown 的纯 JS 网页阅读器雏形
- [x] 理解「数据结构」「事件」「本地存储」三个概念

**reader-layout 原型 · UI 后续（已排期意向，与 SageRead 对齐）**：
- [x] **顶栏多书标签页**（已排入计划 → **实现日 5/26**，见 Week 5 当日任务。此前 MVP1 为单标签占位 `topbarCurrentBookTitle`）：用户从书架依次打开多本书时，顶栏展示多个书名标签，可切换当前阅读上下文（并可扩展关闭标签等）；与 `?book=` 或会话状态同步。与 Lora 约定见 2026-05-11 对话。
...
> **执行记录（5/19 补做）**：`createBlock` 于 5/19 完成并 commit；其余三项函数命名重构仍暂缓。

**操作时段**：
- [x] 把 Week 3 的散乱代码用函数重构：
  - `createBlock(type, content)` ✅ `script.js` 中 `saveStore` 正下方；`onAddNote` 调用 `createBlock("ai_mock", text)`；`seedMockIfEmpty` 等暂不动
  - `saveToLocalStorage()` ⏳ 仍为 `saveStore` / `saveNotes`（功能已有，未按计划命名）
  - `loadFromLocalStorage()` ⏳ 仍为 `loadStore` / `loadNotes`（功能已有，未按计划命名）
  - `exportMarkdown(book)` ⏳ 仍为 `buildExportMarkdown()`（功能已有，未按计划命名）
- [x] 让 Cursor 解释「为什么要用函数」
- [x] `git commit`
...
> **执行记录（5/19）**：同步/异步概念已在 Claude.ai 理解；`setTimeout` / `fetch` 练习 **推迟至 Week 7**（结合 API 实战）。尚硅谷 **P41–P47**（嵌套 for、break/continue、对象入门），当前进度 **P47**。

**学习时段**：MDN 中 Promise 和 async/await 章节
- 这是后续接入 AI API 必备的概念

**操作时段**：
- [x] 理解「同步」和「异步」概念（Claude.ai 完成，不需 Cursor 再讲）
- [x] ~~`setTimeout` 练习~~ → **推迟至 Week 7**（见 6/8 补练项）
- [x] ~~`fetch` 公开 API 练习~~ → **推迟至 Week 7**（见 6/8 补练项）
- [x] `git commit`（本日无新代码，无需提交）
...
- [ ] 把阅读器拆分为四个组件：
  ...
  - `BlockEditor`（笔记块编辑器）
- [ ] 先用假数据渲染三栏界面
- [ ] **顶栏多书标签页**（承接 Week 3 意向）：从书架打开多本书时顶栏展示多个书名标签，可切换当前阅读上下文（可先不做关闭标签）；与当前 `bookId` / 路由或 state 同步
- [ ] 让 Cursor 解释每个组件的职责
- [ ] `git commit`
...
> **自 Week 4（5/19）推迟的预习**（无实际场景时先不做；本周结合 API 一起练）：`setTimeout` 观察异步顺序；`fetch` 调用免费公开 API 小示例。
...
**操作时段**：
- [ ] （补 Week 4）`setTimeout` 练习：观察异步执行顺序
- [ ] （补 Week 4）`fetch` 调用免费公开 API（如天气 API）小示例
